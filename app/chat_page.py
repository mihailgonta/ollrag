...
def ollama_inference(prompt, collection, conversation_key, temperature):
    ...
    st.session_state[conversation_key].append({"content": prompt, "role": "user"})
    ...
    ollama_assistant = OllamaAssistant(
        ...
        rag_config=RAGConfig(n_chunks=10, rerank=False, top_k=10),
    )
...
def send_feedback(trace_id, name, value):
...
def send_user_comment(trace_id, comment: str = None):
    trace = langfuse.trace(id=trace_id)
    trace.update(
        metadata={
    ...
    for idx, message in enumerate(st.session_state[chat_history_key]):
        ...
        if role == "user":
            ...
            with st.chat_message("assistant", avatar="🤖"):
                ...
                feedback_key = (
                    f"feedback_{message.get('trace_id', '')}_{idx}_{chat_history_key}"
                )
                comment_key = (
                    f"comment_{message.get('trace_id', '')}_{idx}_{chat_history_key}"
                )
                ...
                if not message.get("user_comment", False):
                    user_comment = st.chat_input(
                        placeholder="Lasa un feedback",
                        key=comment_key,
                    )

                    if user_comment:
                        message["user_comment"] = True
                        send_user_comment(
                            trace_id=message["trace_id"], comment=user_comment
                        )
...
def get_model_settings():
    model_col, collection_col, rag_toggle_col = st.columns(
        [4, 4, 1], vertical_alignment="bottom"
    )
    with model_col:
        ollama_models = ollama.list()
        selected_model = st.selectbox(
            "Model", [model.model for model in ollama_models.models]
        )
...
st.markdown(
    "<h1 style='text-align: center; color: grey; padding: 2rem 0rem 4rem;'>Ollama RAG 🦙</h1>",
...
ollama_inference(prompt, collection, conversation_key, temperature)
