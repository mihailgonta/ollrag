...
from scripts.prompt_templates import RAG, DIRECT, QUERY_AUGMENTATION
...
class OllamaAssistant:
    ...
    def __get_chat_prompt(self, use_rag: bool):
        template = RAG if use_rag else DIRECT
        return ChatPromptTemplate.from_template(template)

    
    def __augment_query(self, query):
        prompt_template = ChatPromptTemplate.from_template(QUERY_AUGMENTATION)
    ...
    def __embed_and_query(self, query: str) -> List[Document]:
        if not self.embeddings or not self.collection:
            raise ValueError("RAG functionality requires embeddings and collection_name")

        queries = self.__augment_query(query) if self.rag_config.augment_query else [query]
    ...
    def __prepare_response_config(self, user_id: Optional[str], query_type: str) -> dict:
        ...
        if query_type == "rag":
            metadata.update({
                "augmented": self.rag_config.augment_query,
                "reranked": self.rag_config.rerank,
    ...
    def __prepare_chain_input(self, query: str, use_rag: bool) -> Tuple[dict, Optional[List[Document]]]:
        if use_rag:
