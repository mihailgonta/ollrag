...
class OllamaDb:
    ...
    def get_chroma_collection(self, name: str):
        try:
            ...
            raise ValueError(
                f"Collection '{name}' does not exist. Error: {str(e)}"
            ) from e
    ...
    def load_documents(self, file_paths: List[str]) -> List[Document]:
        ...
        for file_path in file_paths:
            if file_path.endswith(".pdf"):
                ...
                document = [
                    Document(
                        page_content=str(page_content), metadata={"source": file_path}
                    )
                ]
    ...
    def chunk_documents(
        self, documents: List[Document], max_chunks: int, min_chunks: int
    ):
        text_splitter = ClusterSemanticChunker(
            self.embeddings.embed_documents,
            max_chunk_size=max_chunks,
            min_chunk_size=min_chunks,
        )
    ...
    def create_collection(self, chunks: List[Document], name: str):
        try:
            ...
            raise ValueError(
                f"Couldn't create collection '{name}'. Error: {str(e)}."
            ) from e
